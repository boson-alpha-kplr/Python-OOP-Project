...
def generate_class_hierarchy(json_dict :dict, superclass_name:str=None,superclass_args:list=[]):
    ...
    for class_name, class_attrs in json_dict.items(): #Itération sur les éléments du dictionnaire
        class_def = generate_class_def(class_name,class_attrs,superclass_name, superclass_args) #définition de la classe + stocker dans une variable 'class_def' 
        class_defs += class_def #Concaténer la définition de la classe à la chaîne de caractères class_defs
        print(class_defs)
        for attribut in class_attrs : # parcourir les elements dans class_attrs pour vérifier si "subclasses" existe parmi les attributs
             if(attribut=="subclasses"): #si "subclasses" existe parmi les attributs
                super_attr=list(class_attrs)+(superclass_args) #Construire une liste "super_attr" contenant les attributs de la classe courante concaténées aux arguments de la superclasse
                super_attr.remove("subclasses")#supprimer l'attribut 'subclasses' à partir de la liste créée
                generate=generate_class_hierarchy(class_attrs.get("subclasses"),class_name,super_attr)#faire une récursion pour générer la définition de la sous-classe en utilisant la méthode generate_class_hierarchy
                class_defs +=generate
...
resultat=generate_class_hierarchy(json_dict)
print(resultat)
write_content(resultat,"product_classes.py")
